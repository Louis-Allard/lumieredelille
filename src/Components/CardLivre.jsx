...
const CardLivre = () => {
  ...
  const fetchLivres = async (page) => {
    try {
      const response = await fetch('http://localhost/Les_lumieres_de_Lille/Backend/afficherLivreCard.php');
        ...
        <div className="book_grid">
          {filteredLivres.map((livre) => (
            <div key={livre.id} className="book_card">
              <img src={`http://localhost/Les_lumieres_de_Lille/Backend/${livre.image_url}`} alt={livre.titre} className="imageCardLivre" />
