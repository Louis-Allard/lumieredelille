...
const DetailLivre = () => {
    ...
    useEffect(() => {
        const fetchLivreDetails = async () => {
            try {
                const response = await fetch(`http://localhost/Les_lumieres_de_Lille/Backend/Api.php?id=${id}`);
                if (!response.ok) {
        ...
        <div className="content">
            {livre ? (
                <div className="cardLivre">
                    <img src={`http://localhost/Les_lumieres_de_Lille/Backend/${livre.image_url}`} alt={livre.titre} className="imageLivre" />
